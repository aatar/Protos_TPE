...
int
read_user_input(int max_size, char *user_input) {
	int index = 0;
	char c = -1;
...
int
process_user_input(char *user_input, const int bytes_read, const int max_size,
	const int proxy_socket_fd) {
	int connection_state = ALIVE;

	if (user_input[bytes_read] == '\n') {
		if (bytes_read == (max_size - 2))
			printf("This protocol does not handle commands larger than %d "
				"characters\n", max_size - 2);
		else
			connection_state = communicate_with_server(proxy_socket_fd,
				user_input, index);
	} else {
...
int main(int argc, char const *argv[]) {
	...
	int bytes_read = 0, max_size = 128, connection_state = ALIVE;
	...
	while (connection_state == ALIVE) {
		bytes_read = read_user_input(max_size, user_input);
		connection_state = process_user_input(user_input, bytes_read, max_size,
			proxy_socket_fd);
	}
